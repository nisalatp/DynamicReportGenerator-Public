...
<template>
  <div class="va-registrar">
    ...
    <div>
      ...
      <textarea v-model="form.sql_fragment" placeholder="(SELECT COALESCE((SELECT COALESCE(SUM(amount),0) FROM orders WHERE orders.user_id = t0.id),0) FROM orders WHERE orders.user_id = t0.id)"></textarea>
    </div>
