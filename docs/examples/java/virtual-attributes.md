...
public class VARegistrationService {
    ...
    public void registerAttribute() throws Exception {
        ...
        RegisterVARequest req = new RegisterVARequest(
            ...
            "Order",
            "integer",
            "(SELECT COALESCE((SELECT COALESCE(SUM(amount),0) FROM orders WHERE orders.user_id = t0.id),0) FROM orders WHERE orders.user_id = t0.id)"
        );
        ...
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create("/api/va/register"))
