...
class ReportMaker
{
     ...
     */
    public function generate(ReportRequest $request, ?array $subjects = null, bool $applyLimit = true): Builder
    {
        ...
        // Callers such as exportToCsv() pass $applyLimit = false to bypass this cap so that
        // cursor-based streaming can export the full dataset without truncation.
        // If the request specifies a per-query limit, it takes precedence over the config default.
        if ($applyLimit) {
            $effectiveLimit = $request->limit
                ?? config('dynamicreportgenerator.limits.max_rows');
            if ($effectiveLimit) {
                $finalQuery->limit((int) $effectiveLimit);
            }
        }
    ...
    {
        // Pass applyLimit = false so the cursor streams the full dataset — not just max_rows.
        $query = $this->generate($whatUserWants, $subjects, applyLimit: false);
