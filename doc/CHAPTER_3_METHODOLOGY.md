...
- **Broker Pattern**: The `ReportMaker` engine acts as an intermediary (broker) for Schema Discovery. It delegates to the `DiscoversSchema` trait to abstract away PHP Reflection and physical database schema lookups, allowing external decoupled frontends to dynamically query the available models and virtual attributes via standard APIs.
